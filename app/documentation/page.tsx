...
function DocumentationContent() {
  ...
  const clearHistory = () => {
    ...
    toast({
      title: t('documentation.toast.history_cleared'),
      description: t('documentation.toast.history_cleared_desc'),
    });
  };
  ...
  const handleTemplateSelection = (template: any) => {
    ...
    // Show success toast
    toast({
      title: t('documentation.toast.template_applied'),
      description: t('documentation.toast.template_applied_desc', { name: template.name }),
    });
  };
  ...
  const sendRequest = async () => {
    if (!agentUrl) {
      toast({
        title: t('documentation.toast.agent_url_required'),
        description: t('documentation.toast.agent_url_required_desc'),
        variant: "destructive",
      });
      return;
    }
    ...
    if (result.status) {
      ...
      if (status.message && status.message.parts) {
        ...
        if (textParts.length > 0) {
          ...
          addDebugLog(
            `A2A message text: "${currentMessageText.substring(0, 50)}${currentMessageText.length > 50 ? "..." : ""
            }"`
          );
      ...
      result.artifacts.forEach((artifact: any, index: number) => {
        if (artifact.parts && artifact.parts.length > 0) {
          ...
          if (textParts.length > 0) {
            ...
            addDebugLog(
              `A2A Artifact ${index + 1} text: "${artifactText.substring(0, 50)}${artifactText.length > 50 ? "..." : ""
              }"`
            );
  ...
  const copyToClipboard = (text: string) => {
    ...
    toast({
      title: t('documentation.toast.copied'),
      description: t('documentation.toast.copied_desc'),
    });
  };
      ...
      case "submitted":
        return (
          <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
            {t('documentation.status.submitted')}
          </span>
        );
      case "working":
        return (
          <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
            {t('documentation.status.processing')}
          </span>
        );
      case "completed":
        return (
    ...
    <div className="container mx-auto p-6 bg-[#121212] min-h-screen">
      ...
      <div className="text-center mb-8">
        <div className="flex justify-center mb-4">
          <div className="flex items-center space-x-3 bg-gradient-to-r from-blue-500/20 to-blue-500/20 px-6 py-3 rounded-full border border-blue-500/30">
            <Network className="h-6 w-6 text-blue-400" />
            <span className="font-bold text-blue-400">A2A Protocol</span>
            <span className="text-xs bg-blue-500/20 px-2 py-1 rounded text-blue-300">v0.2.1</span>
          </div>
        </div>
        <h1 className="text-4xl font-bold bg-gradient-to-r from-blue-400 via-blue-400 to-purple-400 bg-clip-text text-transparent mb-4">
      ...
      >
        <TabsList className="bg-[#121212] w-full mb-6 p-1 rounded-lg grid grid-cols-3">
          <TabsTrigger
            value="docs"
            className="data-[state=active]:bg-blue-500/20 data-[state=active]:text-blue-400 data-[state=active]:border-blue-500/50 flex items-center justify-center space-x-2 px-4 py-2 rounded-md transition-all"
          >
            <BookOpen className="h-4 w-4" />
            <div className="text-center">
              <div className="font-medium text-sm">{t('documentation.tabs.docs')}</div>
              <div className="text-xs opacity-70">{t('documentation.tabs.docs_subtitle')}</div>
            </div>
          </TabsTrigger>
          <TabsTrigger
            value="lab"
          ...
          >
            ...
            <div className="text-center">
              <div className="font-medium text-sm">{t('documentation.tabs.lab')}</div>
              <div className="text-xs opacity-70">{t('documentation.tabs.lab_subtitle')}</div>
            </div>
          </TabsTrigger>
          <TabsTrigger
            value="examples"
          ...
          >
            ...
            <div className="text-center">
              <div className="font-medium text-sm">{t('documentation.tabs.examples')}</div>
              <div className="text-xs opacity-70">{t('documentation.tabs.examples_subtitle')}</div>
            </div>
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="lab">
          ...
          <div className="mb-6">
            ...
            >
              <h3 className="text-blue-400 font-semibold flex items-center text-sm">
                🔧 Active A2A Configuration
                <span className="ml-2 text-blue-300 text-xs">(Real-time feature status)</span>
              </h3>
              <div className="flex items-center space-x-2">
                <span className="text-xs text-neutral-500">
            ...
            {showConfigStatus && (
              <Card className="bg-gradient-to-r from-blue-500/5 to-blue-500/5 border-blue-500/20 text-white">
                <CardContent className="p-4">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div className="bg-[#222]/50 p-3 rounded-lg">
                      <h4 className="text-blue-400 font-medium mb-2">Multi-turn Conversations</h4>
          ...
          <Card className="bg-[#1a1a1a] border-[#333] text-white mb-6">
            <CardHeader>
              <CardTitle className="text-blue-400">{t('documentation.lab.title')}</CardTitle>
              <CardDescription>
                {t('documentation.lab.description')}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Tabs defaultValue="http" onValueChange={setLabMode}>
                <TabsList className="bg-[#222] border-[#333] mb-4">
                  <TabsTrigger
                    value="http"
                    className="data-[state=active]:bg-[#333] data-[state=active]:text-blue-400"
                  >
                    {t('documentation.lab.http_request')}
                  </TabsTrigger>
                  <TabsTrigger
                    value="stream"
                    className="data-[state=active]:bg-[#333] data-[state=active]:text-blue-400"
                  >
                    {t('documentation.lab.streaming')}
                  </TabsTrigger>
                </TabsList>
          ...
          {response && labMode === "http" && (
            <Card className="bg-[#1a1a1a] border-[#333] text-white">
              <CardHeader>
                <CardTitle className="text-blue-400">{t('documentation.lab.response')}</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card className="bg-[#1a1a1a] border-[#333] text-white">
              <CardContent className="p-6 text-center">
                <p className="text-neutral-400">
                  {t('documentation.lab.click_send')}
                </p>
              </CardContent>
            </Card>
          )}
            ...
            <Card className="bg-[#1a1a1a] border-[#333] text-white">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle className="text-blue-400">{t('documentation.lab.debug_logs')}</CardTitle>
                  <div className="flex gap-2">
                    ...
                    >
                      {showDebug ? t('documentation.lab.hide_logs') : t('documentation.lab.show_logs')}
                    </Button>
                    <Button
                      variant="outline"
                      size="sm"
                      ...
                      className="bg-[#222] border-[#444] text-white hover:bg-[#333]"
                    >
                      {t('documentation.lab.clear')}
                    </Button>
                  </div>
                </div>
                <CardDescription>
                  {t('documentation.lab.debug_subtitle')}
                </CardDescription>
              </CardHeader>
              {showDebug && (
                <CardContent>
      ...
      <Card className="bg-gradient-to-r from-blue-500/5 to-blue-500/5 border-blue-500/20 text-white mt-12">
        <CardContent className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
            <div>
              <h3 className="font-semibold text-blue-400 mb-2">{t('documentation.footer.official_resources')}</h3>
              <div className="space-y-2 text-sm">
                ...
                >
                  📋 {t('documentation.footer.official_spec')}
                </a>
                <a
                  ...
                  className="block text-blue-400 hover:text-blue-300 transition-colors"
                >
                  🔗 {t('documentation.footer.github_repo')}
                </a>
              </div>
            </div>
            <div>
              <h3 className="font-semibold text-blue-400 mb-2">{t('documentation.footer.implementation_status')}</h3>
              <div className="space-y-1 text-sm text-neutral-300">
                <div>✅ {t('documentation.footer.compliant')}</div>
                <div>✅ {t('documentation.footer.core_methods')}</div>
                <div>✅ {t('documentation.footer.enterprise_ready')}</div>
              </div>
            </div>
            <div>
              <h3 className="font-semibold text-blue-400 mb-2">{t('documentation.footer.need_help')}</h3>
              <div className="space-y-1 text-sm text-neutral-300">
                <div>📖 {t('documentation.footer.check_docs')}</div>
                <div>🧪 {t('documentation.footer.test_lab')}</div>
                <div>💡 {t('documentation.footer.view_examples')}</div>
              </div>
            </div>
          </div>
          <div className="border-t border-blue-500/20 mt-6 pt-4 text-center text-xs text-neutral-400">
            {t('documentation.footer.built_with')}
          </div>
        </CardContent>
      </Card>
    </div>
