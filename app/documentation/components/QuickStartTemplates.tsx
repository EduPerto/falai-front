...
export function QuickStartTemplates({ onSelectTemplate }: QuickStartTemplatesProps) {
  ...
  const getMethodColor = (method: string) => {
    switch (method) {
      case 'message/send': return 'bg-blue-500/20 text-blue-400 border-blue-500/30';
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {templates.map((template) => {
            ...
            return (
              <div
                key={template.id}
                className="bg-[#222] border border-[#444] rounded-lg p-4 hover:border-blue-500/50 transition-colors cursor-pointer group"
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    size="sm"
                    variant="ghost"
                    className="text-blue-400 hover:text-blue-300 hover:bg-blue-500/10 text-xs px-2 py-1 h-auto opacity-0 group-hover:opacity-100 transition-opacity"
                  >
                    Use Template
                  </Button>
                </div>
